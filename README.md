...
* **⌚ 抬腕直接說（Raise to Speak）與靜默看錶體驗（Stealth Glance）**：
  * **0% 微光耗電（Zero AOD Drain）**：螢幕暗下或微光模式時，硬體麥克風完全關閉釋放，零額外後台耗電。
  * **抬手靜默看錶（Stealth Glance Mode）**：抬手僅看時間時，**0 光圈、0 震動、無任何畫面遮擋**，保持最極致乾淨的日常看錶體驗！3.5 秒靜態監聽視窗內若未開口說話，麥克風安靜自動釋放。
  * **開口即亮光圈與震動確認（Lookback Buffer）**：3.5 秒內偵測到真正開口說話時，手錶立即反饋清脆震動並亮起彩色旋轉霓虹光圈；透過記憶體內 160ms 環形預錄緩衝區，確保使用者開口的第一個字音（如「開」、「打」、「幫」）100% 完整保留不截斷。
  * **防抓頭髮與衣物摩擦雜音過濾（Anti-Friction & Noise Rejection）**：
    - 結合連續多訊框語音判定（需持續約 120ms~160ms 語音能量）與高頻過零率（ZCR）排除演算法，精準過濾抓頭髮、衣物摩擦或觸碰麥克風孔之瞬態高頻雜音。
    - 最短有效語音保護（總說話時長 < 600ms 自動安靜捨棄），配合收緊的離線正則與 Gemini 雜音守則，徹底杜絕抓頭髮被誤判為開手電筒或亂輸入。
  * **雙擊備用手勢**：手錶已亮著時，快速**雙擊錶面任意處**亦可隨時立即震動並召喚助理。
...
* **應用程式抽屜 (App Drawer Overlay)：**
  ...
  * **🔄 雙軌虛擬圓周錶圈輪轉 (Dual-Track Rotary Touch Bezel)**：
    * **外圈圓周滑動手勢**：嚴格限定於螢幕外周 70%~130% 環形區域（貼著 Galaxy Watch 5 Pro 凸起的鈦金屬內凹外框邊緣畫圓），順時針向下滾動、逆時針向上滾動，視野完全不被大拇指遮擋！
    * **中央 70% 防誤觸保護**：螢幕中央 70% 區域完全留給正常的上下垂直滑動與按鍵點擊，絕不因直線滑動誤觸轉圈。
    * **機械齒輪微震動 (Haptic Click Wheel)**：每轉動約 6.8 度觸發一次清脆的線性馬達刻度微震，帶來如同實體機械旋轉錶圈的咔嗒節奏反饋。
  ...
  * **🔝 常駐頂部一鍵秒回頂部 (Pinned Tap-to-Top)**：頂部「APPS (數量)」膠囊**固定置頂**，永不隨清單滾走。滑到最底部時徽章自動轉化為「`↑ APPS`」，隨時輕觸即可秒速平滑滑回最頂端！
